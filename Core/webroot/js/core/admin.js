...
Admin.form = function() {
  // Tooltips activation
  $(
    "[rel=tooltip],*[data-title]:not([data-content]),input[title],textarea[title]"
  ).tooltip();
...
Admin.protectForms = function() {
  ...
  if (forms.length > 0) {
    ...
    window.onbeforeunload = function(e) {
      ...
      if (!dirty) {
        if (
          typeof Croogo.Wysiwyg.isDirty == "function" &&
          !Croogo.Wysiwyg.isDirty()
        ) {
          return;
        } else {
          return;
  ...
  $("body").on("submit", "form", function(el) {
    ...
    if (el.originalEvent.explicitOriginalTarget) {
      ...
      if ($button.find("i").length == 0) {
        $button.prepend(" ").prepend($("<i />").addClass(Admin.spinnerClass()));
      } else {
...
Admin.slideBoxToggle = function() {
  ...
  $("body").on("click", ".box-title", function() {
    $(this)
      ...
      .end()
      .find(iconMinus)
      .switchClass(iconMinus, iconPlus)
      .end()
      .find(iconPlus)
